...
fn expensive_sum(v: Vec<i32>) -> i32 {
    ...
    v.iter()
        ...
        // .map() goes here
        .map(|&even_number| { even_number * even_number })
        .sum()
}
...
fn main() {
    ...
    //
    let sum = handle.join().expect("no value returned");
    ...
    let handle_1 = thread::spawn(move || {
        for message in reciever {
            println!("thread 1 value is: {}", message);
        }
    });

    let handle_2 = thread::spawn(move || {
        for message in reciever2 {
            println!("thread 2 value is: {}", message);
        }
    });
